...
## Version 21.0.0 (2026-04-01)

- chore: upgrade Tutor compatibility to `21.x`
- chore: use temporary Panorama backend and MFE git tags for the Ulmo rollout
...
- Feat: Optimize Fluent Bit log file batching to reduce S3 file count
  ...
  - Updated `PANORAMA_LOGS_UPLOAD_TIMEOUT` default from `15m` to `10m`
  - Added new `PANORAMA_LOGS_UPLOAD_CHUNK_SIZE` setting (default `10M`) to control multipart upload chunk size
  - These changes reduce the number of S3 files generated by approximately 50-100x, improving storage efficiency and reducing costs
- Feat: Use plugin slots to add Panorama to the learner dashboard MFE
- Chore: Add GitHub Actions for package validation on push and PyPI publishing on GitHub releases
- Upgrade Tutor compatibility to `20.x.x`
- Use `openedx-backend-version` `20.0.0`
- Update Panorama MFE to `open-release/teak/v202603170221`
